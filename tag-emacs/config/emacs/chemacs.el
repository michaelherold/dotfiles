...
;; NOTE I have customized chemacs a little bit to make it conform to my
;; sensibilities.

;;; Code:
(defvar chemacs-version "2.0")
(defvar config-home (or (getenv "XDG_CONFIG_HOME") "~/.config"))
(defvar chemacs-profiles-path (format "%s/emacs/profiles.el" config-home))
(defvar chemacs-default-profile-path (format "%s/emacs/profile" config-home))
