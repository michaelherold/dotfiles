...
The automatic pairing for smartparens' =`= pair gets applied within Org mode when it shouldn't. Also, the built-in pair for the =~= key is not working, which makes it harder to use the formatting for code. Lastly, the ='= pair is irritating when I am modifying my literate configuration because it tries to pair when I'm writing elisp.
